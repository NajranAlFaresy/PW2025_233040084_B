...
<div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center gap-4 from-blue-50 to-indigo-50">
        ...
        <nav aria-label="Page navigation">
            <ul class="flex space-x- px- text-sm">
                ...
                @if ($posts->onFirstPage())
                    <li>
                        <span class="flex items-center justify-center text-gray-400 bg-gray-100 box-border border border-gray-300 cursor-not-allowed font-medium rounded-base text-sm px-3 h-10">
                            Previous
                        </span>
                    </li>
                @else
                    <li>
                        <a href="{{ $posts->previousPageUrl() }}" class="flex items-center justify-center text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-base text-sm px-3 h-10 focus:outline-none">
                            Previous
                        </a>
                    </li>
                @endif
                    ...
                    @if($page == $posts->currentPage())
                        <li>
                            <a href="{{ $url }}" aria-current="page"
                                class="flex items-center justify-center text-heading bg-neutral-tertiary-medium box-border border border-default-medium hover:text-bg-brand font-medium text-sm w-10 h-10 focus:outline-none">
                                {{ $page }}
                            </a>
                        </li>
                    @else
                ...
                <td class="px-6 py-4">
                    ...
                    @else
                        <img id="preview" 
                            class="w-32 h-32 rounded-base object-cover border border-default bg-gray-100" 
                ...
                @if($posts->hasMorePages())
                    <li>
                        <a href="{{ $posts->nextPageUrl() }}" class="flex items-center justify-center text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-base text-sm px-3 h-10 focus:outline-none">
                            Next
                        </a>
                    </li>
                @else
